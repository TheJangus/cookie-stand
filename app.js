...
CookieShop.prototype.cookiesPerHr = function () {
    for (let i = 0; i < this.customer.length; i++) {
        this.cookies[i] = Math.round(this.customer[i] * this.cookiesPerSale);
...
function renderAll() {
    ...
    for (let i = 0; i < allCookieShops.length; i++) {
        allCookieShops[i].custPerHour();
        allCookieShops[i].cookiesPerHr();
...
function createFooter() {
    ...
    trElem.appendChild(tdElem);
    locationTable.appendChild(trFoot);
    ...
    for (let i = 0; i < operatingHours.length; i++) {
        ...
        for (let j = 0; j < allCookieShops.length; j++) {
            hourlyTotal += allCookieShops[j].cookies[i];
            grandTotal += allCookieShops[j].cookies[i];
        }
        let td2Elem = document.createElement("td");
        trElem.appendChild(td2Elem);
        td2Elem.textContent = hourlyTotal;
    }
}
...
function addNewCookieStore(event) {
    ...
    if (!event.target.name.value || !event.target.minCustomers.value || !event.target.maxCustomers.value || !event.target.cookiesPerSale.value) {
        alert('Oops, I tend to forget things too! Please fill out all blocks and try again!');
    }
    if (event.target.minCustomers.value < 0 || event.target.maxCustomers.value < 0 || event.target.cookiesPerSale.value < 0) {
        alert('Just how does one have negative customers? And free cookies, we would go out of business!');
    }
    if (event.target.minCustomers.value > event.target.maxCustomers.value) {
        alert('Me thinks you got those numbers backwards');
    }
    ...
    for (let i = 0; i < allCookieShops.length; i++) {
        if (newShopName == allCookieShops[i].location) {
            ...
            allCookieShops[i].cookiesPerSale = cookiesPerSale;
            renderAll();
            return;
        } else {
            ...
            new CookieShop(newShopName, addMinCustomers, addMaxCustomers, cookiesPerSale);
            renderAll();
            return;
        }
    }
...
//Event Listener
cookieShopForm.addEventListener('submit', addNewCookieStore);
